...
const fetchWarehouseReceivingHistoryApi = async (dateFrom, dateTo) => {
    const res = await request.get(`Reports/WareHouseReceivingReports?dateFrom=${dateFrom}&dateTo=${dateTo}`)
    return res.data
  }
          ...
          <PageScroll minHeight='600px' maxHeight='620px'>
            <Table size='sm' variant="striped">
              <Thead bgColor='primary' h="40px" >
                <Tr>
                  <Th color='white' fontSize="11px" fontWeight="semibold">ID</Th>
                  <Th color='white' fontSize="11px" fontWeight="semibold">Received Date</Th>
                  <Th color='white' fontSize="11px" fontWeight="semibold">PO Number</Th>
                  {
                    buttonChanger ?
                      <>
                      ...
                      <>
                        ...
                        <Th color='white' fontSize="11px" fontWeight="semibold">Total Reject</Th>
                        <Th color='white' fontSize="11px" fontWeight="semibold">Supplier</Th>
                        <Th color='white' fontSize="11px" fontWeight="semibold">Transaction Type</Th>
                        <Th color='white' fontSize="11px" fontWeight="semibold">Received By</Th>
                      </>
                  }
                </Tr>
              </Thead>
              <Tbody>
                {
                  warehouseData?.map((item, i) =>
                    <Tr key={i}>
                      <Td fontSize="12px">{item.warehouseId}</Td>
                      <Td fontSize="12px">{item.receiveDate}</Td>
                      <Td fontSize="12px">{item.poNumber ? item.poNumber : ''}</Td>
                      {
                        buttonChanger
                          ?
                          <>
                            <Td fontSize="12px">{item.itemCode}</Td>
                            <Td fontSize="12px">{item.itemDescrption}</Td>
                            <Td fontSize="12px">{item.uom}</Td>
                            <Td fontSize="12px">{item.category ? item.category : 'Miscellaneous'}</Td>
                          ...
                          <>
                            ...
                            <Td fontSize="12px">{item.totalReject}</Td>
                            <Td fontSize="12px">{item.supplierName}</Td>
                            <Td fontSize="12px">{item.transactionType}</Td>
                            <Td fontSize="12px">{item.receivedBy}</Td>
                          </>
                      }
                    </Tr>
                  )
