...
export const ListMoveOrder = ({ moveOrderList, setMoveOrderInformation, moveOrderInformation, moveOrderViewTable, checkedItems, setCheckedItems, status }) => {
    const { isOpen: isView, onClose: closeView, onOpen: openView } = useDisclosure()

    const viewHandler = ({ orderNo, deliveryStatus, customerName, customerCode }) => {
...
<>
      <Flex w='full' flexDirection='column'>
        <VStack spacing={0}>
          ...
          <PageScroll minHeight='510px ' maxHeight='520px'>
            <Table size='sm' variant='simple'>
              <Thead bgColor='secondary' >
                <Tr>
                  ...
                  <Th color='white' fontSize="11px">Order Id</Th>
                  <Th color='white' fontSize="11px">Customer Code</Th>
                  <Th color='white' fontSize="11px">Customer Name</Th>
                  <Th color='white' fontSize="11px">Category</Th>
                  <Th color='white' fontSize="11px">Total Quantity Order</Th>
                  {/* <Th color='white'>Order Date</Th> */}
                  <Th color='white' fontSize="11px">Date Needed</Th>
                  <Th color='white' fontSize="11px">Prepared Date</Th>
              ...
              <Tbody>
                {
                  moveOrderList?.map((list, i) =>
                    ...
                    >
                      ...
                      <Td fontSize="11px">{moment(list.preparedDate).format('MM/DD/yyyy')}</Td>
                      <Td fontSize="11px">
                        <Button size='xs' colorScheme='blue' borderRadius="none" onClick={() => viewHandler(list)}>
                          View
                        </Button>
                      </Td>
                    </Tr>
