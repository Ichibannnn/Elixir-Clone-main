...
import {
    ...
    Table, Tbody, Td, Th, Thead, Tr, ModalOverlay, FormLabel, Stack,
            ...
            <ModalBody mt={5}>
                ...
                <Flex w='full' flexDirection='column' borderX='1px'>
                    <VStack w='full' spacing={0} mb={6}>
                        <Text w='full' textAlign='center' bgColor='primary' color='white' fontSize='xs'>Transact Move Order</Text>
                        <Text w='full' textAlign='center' bgColor='secondary' color='white' fontSize='xs'>Move Order Information</Text>
                        <VStack w='99%' mb={6}>
                            <Stack w='full' mt={2}>
                                <Flex justifyContent="space-between">
                                    <FormLabel fontSize='xs' w="40%">
                                        Order ID:
                                        <Text textAlign='center' w='full' fontSize='xs' bgColor='gray.200' border='1px' py={1}>
                            ...
                            <Stack w='full' mt={2}>
                                <Flex justifyContent="space-between">
                                    <FormLabel fontSize='xs' w="40%">
                                        Customer Code:
                                        <Text textAlign='center' w='full' fontSize='xs' bgColor='gray.200' border='1px' py={1}>
                                            {moveOrderInformation.customerName ? moveOrderInformation.customerName : 'Please select a list'}
                                        </Text>
                                    </FormLabel>
                                    ...
                                    <FormLabel fontSize='xs' w="40%">
                                        Customer Name:
                                        <Text textAlign='center' w='full' fontSize='xs' bgColor='gray.200' border='1px' py={1}>
                                            {moveOrderInformation.customerCode ? moveOrderInformation.customerCode : 'Please select a list'}
                                        </Text>
                                    </FormLabel>
                                </Flex>
                            </Stack>
                ...
                <Flex w='full' flexDirection='column' border='1px'>
                    <VStack spacing={0}>
                        <Text pb={2} textAlign='center' fontSize='xs' color='white' bgColor='primary' w='full' mb={-1.5}>List of Move Orders</Text>
                            ...
                            <Table size='sm' variant='simple'>
                                <Thead bgColor='secondary'>
                                    <Tr>
                                        {
                                            TableHead?.map((t, i) =>
                                                <Th color='white' fontSize="10px" key={i}>{t}</Th>
                                            )
                                        }
                                    </Tr>
                                </Thead>
...
export const TransactConfirmation = ({ isOpen, onClose, deliveryDate, checkedItems, setCheckedItems, fetchMoveOrderList, setDeliveryDate }) => {
    ...
    const submitHandler = () => {
        ...
        setIsLoading(true)
        try {
            const res = request.post(`Ordering/TransactListOfMoveOrders`, arraySubmit)
                .then(res => {
