...
import { Button, ButtonGroup, Flex, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, Text, Table, Tbody, Td, Th, Thead, Tr, useToast, VStack, HStack, ModalOverlay } from '@chakra-ui/react'
        ...
        <Modal isOpen={isOpen} onClose={() => { }} size='5xl' isCentered>
            ...
            <ModalContent>
                <ModalHeader mt={5} fontSize='md'>
                    <Flex fontSize='sm' justifyContent='center'><Text>Issue Details</Text></Flex>
                    <Flex justifyContent='space-between'>
                        <VStack alignItems='start' spacing={-1}>
                            <Text fontSize="xs">Customer Code: {issuesDetailsData[0]?.customerCode}</Text>
                            <Text fontSize="xs">Customer Name: {issuesDetailsData[0]?.customer}</Text>
                            <Text fontSize="xs">Details: {issuesDetailsData[0]?.remarks}</Text>
                        </VStack>
                        <VStack alignItems='start' spacing={-1}>
                            <Text fontSize="xs">Transaction ID: {issuesDetailsData[0]?.issuePKey}</Text>
                            <Text fontSize="xs">Transaction Date: {moment(issuesDetailsData[0]?.preparedDate).format('yyyy-MM-DD')}</Text>
                            <Text fontSize="xs">Transact By: {issuesDetailsData[0]?.preparedBy}</Text>
                        </VStack>
                    </Flex>
                </ModalHeader>
                            ...
                            <Table size='sm'>
                                <Thead bgColor='secondary'>
                                    <Tr>
                                        ...
                                        <Th color='white' fontSize="xs">Quantity</Th>
                                    </Tr>
                                </Thead>
                                <Tbody>
                                    {
