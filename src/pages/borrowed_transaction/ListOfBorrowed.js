...
export const ListOfBorrowed = ({ borrowedData, selectorId, setSelectorId, setTotalQuantity, fetchActiveBorrowed, fetchBarcodeNo, remarks }) => {
  const TableHead = [
  ...
  return (
    <Flex justifyContent='center' flexDirection='column' w='full'>
        <VStack justifyContent='center' w='full' spacing={-1}>
            ...
            <Text bgColor='secondary' fontSize="xs" w='full' color='white' textAlign='center' fontWeight='semibold' py={1}>List of Borrowed Materials</Text>
            <Flex justifyContent='center' w='full'>
                <PageScroll minHeight='280px' maxHeight='300px'>
                    <Table size='sm'>
                        <Thead bgColor='secondary'>
                            <Tr>
                                {TableHead?.map((item, i) => <Th color='white' fontSize="11px" key={i}>{item}</Th>)}
                            </Tr>
                        </Thead>
                        <Tbody>
                            {
