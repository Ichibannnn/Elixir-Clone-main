...
import { Button, ButtonGroup, Flex, HStack, Input, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Select, Spinner, Text, useDisclosure, VStack } from '@chakra-ui/react'
...
export const BorrowedInformation = ({ rawMatsInfo, setRawMatsInfo, details, setDetails, customerRef,
    customers, rawMats, uoms, barcodeNo, setSelectorId, setCustomerData, setWarehouseId, warehouseId, fetchActiveBorrowed, customerData, remarks, setRemarks, remarksRef
}) => {
    ...
    return (
        ...
        <VStack w='full' spacing={6}>
            <Text bgColor='secondary' w='full' color='white' textAlign='center' fontWeight='semibold' fontSize="sm">Materials Information</Text>
            <Flex w='95%' justifyContent='space-between'>
                ...
                <VStack alignItems='start' w='40%' mx={5}>
                    ...
                    <HStack w='full'>
                        ...
                        <Text fontSize="xs" pl={2} w='full' border='1px' bg="gray.300" borderColor='gray.200' py={1.5}>{rawMatsInfo.customerName ? rawMatsInfo.customerName : 'Select a customer'}</Text>
                    </HStack>
...
export const RawMatsInfoModal = ({ isOpen, onClose, details, setDetails, rawMatsInfo, setRawMatsInfo,
    customerRef, rawMats, barcodeNo, setSelectorId, setCustomerData, setWarehouseId, warehouseId,
    fetchActiveBorrowed, customerData, remarks, setRemarks
}) => {
                    ...
                    <ModalBody mb={5}>
                        <Flex w='95%' justifyContent='space-between'>
                            <VStack alignItems='start' w='40%' mx={5}>
                                ...
                                {/* Barcode No */}
                                <HStack w='full'>
                                    <Text minW='50%' w='auto' bgColor='secondary' color='white' pl={2} pr={7} py={2.5} fontSize='xs'>Barcode Number: </Text>
                                    {
                                        barcodeNo.length > 0 ?
                                            <Select
                                                fontSize="xs"
                                            ...
                                            >
                                                {
                                                    barcodeNo?.map((item, i) =>
                                                        <option key={i} value={JSON.stringify(item)}>{item.warehouseId}</option>
                                                    )
                                                }
                                            </Select>
                            ...
                            <VStack alignItems='start' w='40%' mx={5}>
                                ...
                                <HStack w='full'>
                                    ...
                                    <Text textAlign='center' fontSize="xs" w='full' bgColor='gray.200' border='1px' borderColor='gray.200' py={1.5}>{rawMatsInfo.itemDescription ? rawMatsInfo.itemDescription : 'Select an item code'}</Text>
                                </HStack>

                                {/* UOM */}
                                < HStack w='full'>
                                    <Text minW='50%' w='auto' bgColor='secondary' color='white' pl={2} pr={7} py={2.5} fontSize='xs'>UOM: </Text>
                                    <Text textAlign='center' fontSize="xs" w='full' bgColor='gray.200' border='1px' borderColor='gray.200' py={1.5}>{rawMatsInfo.uom ? rawMatsInfo.uom : 'Select an item code'}</Text>
                                </HStack>
                    ...
                    <ModalFooter>
                        <ButtonGroup size='xs'>
                            <Button
                                ...
                                disabled={
                                    !rawMatsInfo.itemCode || !rawMatsInfo.customerName || !rawMatsInfo.uom || !rawMatsInfo.warehouseId ||
                                    !rawMatsInfo.quantity || !details || rawMatsInfo.quantity > availableStock
                                }
